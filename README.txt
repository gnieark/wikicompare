...
-The evaluation object, which represent the support of a criteria in a product. It has :
	-A feature and a product entityreference fields to link the evaluation to the feature and product which created it
	-A description
	-A boolean indicating if the feature is supported or not by the product
...
We can't create manually an evaluation, they are automatically created when we create a new feature or a new product.
...
You can also modify the table by clicking on "Toogle Fastaction" which will add some ajax items after the products/features/evaluation. With these items, you can add, edit or remove a node directly in the comparative table. A form will be displayed with a confirm button and the table will be updated after submit.
...
My only left concern about performance is the evaluation creation. When you try to create a product when there are 5000 features, you can expect several minutes of computation because it will create the 5000 evaluation needed, and then update the table with theses 5000 evaluations as argument of the function. Most of the time is taken by the update table function through, not the evaluation creation which takes 30seconds, so maybe we have some hope if we can still improve the update function.

Because of this, I consider the current limit of the system start appear with 5000 nodes on one axis. This is enough to start the project, but we'll need to be able to manage millions of feature/product to be really comfortable. Any audit of the code from a good DBA will be more than welcome.
